...
public class VBusinessLoanValidator implements Validator{
...
*/
    	...
    	}else
    	{
    	 ...
    	 else if(!tenture.matches("^[0-9]+$"))
    	 {
    		...
    		String af=loan.getAuditedfinance();
    		if(af==null||af=="" ||af.matches("[\\s]+"))
    		{
    			errors.rejectValue("auditedfinance", "auditedfinance","Audited Finance is required");

    		}
    		String cap=loan.getComaddressproof();
    		if(cap==null||cap=="" ||cap.matches("[\\s]+"))
    		{
    			errors.rejectValue("comaddressproof", "comaddressproof","Company Address Proof is required");

    		}
    		String cp=loan.getComprofile();
    		if(cp==null||cp=="" ||cp.matches("[\\s]+"))
    		{
    			errors.rejectValue("comprofile", "comprofile","Company Profile is required");

    		}
    		String pp=loan.getProductprofile();
    		if(pp==null||pp=="" ||pp.matches("[\\s]+"))
    		{
    			errors.rejectValue("productprofile", "productprofile","Product Profile is required");
